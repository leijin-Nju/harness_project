...
1. **[Must] 作为项目开发者，我希望用 CLI 启动一次 coding agent 任务，以便让 harness 在受控工作区内完成小型代码修改。**
...
2. **[Must] 作为项目开发者，我希望危险动作不会被 agent 直接执行，以便避免删除文件、泄露凭据或越权修改工作区。**
...
3. **[Must] 作为审批者，我希望通过 CLI 或 WebUI 查看并处理待审批动作，以便在人类确认后才允许中风险操作继续。**
...
4. **[Must] 作为项目开发者，我希望测试和 lint 失败能被结构化回灌给 agent，以便 agent 根据客观反馈修正下一步动作。**
...
5. **[Must] 作为课程评审者，我希望无需真实 LLM 就能运行核心机制演示，以便验证项目机制不是只靠提示词完成。**
...
6. **[Should] 作为项目开发者，我希望 harness 记住项目约定和历史决策，以便后续任务能按需获得相关上下文。**
...
7. **[Should] 作为新用户，我希望安全配置 OpenAI-compatible API key，以便真实 LLM 模式能运行且凭据不泄露。**
...
8. **[Should] 作为部署者，我希望用 Docker 启动 CLI/WebUI，以便在新机器上复现项目运行环境。**
...
9. **[Could] 作为项目开发者，我希望通过最小 WebUI 查看任务状态、反馈摘要、审批队列和记忆条目，以便更直观看到 harness 运行过程。**
...
10. **[Won't for MVP] 作为高级用户，我希望所有命令都在强隔离容器沙箱中执行，以便获得更强的系统级隔离。**
